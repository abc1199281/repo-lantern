...
def _make_llm_mock(content: str = "Generated LLM content") -> MagicMock:
    """Create an LLM mock that returns the same content for every invoke call."""
    llm = MagicMock()
    response = MagicMock()
...
def _make_llm_mock_sequential(contents: list[str]) -> MagicMock:
    """Create an LLM mock that returns different content on successive calls."""
    llm = MagicMock()
    responses = []
    for c in contents:
        r = MagicMock()
...
class TestAgenticSynthesizer:
    ...
    def test_generate_top_down_docs_produces_all_files(
        ...
        llm = _make_llm_mock("LLM generated architecture analysis content.")
    ...
    def test_documents_contain_llm_output(
        ...
        llm = _make_llm_mock("Unique LLM content marker XYZ123.")
        synth = AgenticSynthesizer(tmp_path, llm, language="en")
        synth.generate_top_down_docs()
    ...
    def test_architecture_includes_dependency_graph(
        ...
        llm = _make_llm_mock("Architecture body.")
        synth = AgenticSynthesizer(tmp_path, llm, language="en")
        synth.generate_top_down_docs()
    ...
    def test_sequential_node_context_building(
        ...
        # Check that later calls received earlier outputs in their messages.
        ...
        user_msg_content = second_call_messages[1].content  # HumanMessage
        assert "Factory pattern" in user_msg_content
    ...
    def test_language_parameter_passed_to_prompts(
        ...
        # Check first call's system message includes zh-TW
        first_call_messages = llm.invoke.call_args_list[0][0][0]
        system_msg = first_call_messages[0].content  # SystemMessage
    ...
    def test_doc_truncation(
        ...
        llm = _make_llm_mock(long_content)
        synth = AgenticSynthesizer(tmp_path, llm, language="en")
        synth.generate_top_down_docs()
