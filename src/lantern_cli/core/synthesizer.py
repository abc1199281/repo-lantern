...
class Synthesizer:
    ...
    def _generate_top_down_structured(self) -> None:
        ...
        self._finalize_flat_output()

    def _synth_filename(self, legacy_name: str) -> str:
        """Return the flat or legacy filename for a synthesis doc."""
        if not self.plan:
            return legacy_name
        manifest = build_manifest(self.plan)
        # Map legacy name (without .md) to flat name
        key = legacy_name.replace(".md", "")
        for entry in manifest:
            if entry.original_path == key:
                return entry.flat_name
        return legacy_name
    ...
    def _generate_overview(self, records: list[dict[str, Any]]) -> None:
        ...
        self._write_doc(
            self._synth_filename("OVERVIEW.md"),
            header + self._extract_section(records, "summary"),
        )
    ...
    def _generate_architecture(self, records: list[dict[str, Any]]) -> None:
        ...
        self._write_doc(
            self._synth_filename("ARCHITECTURE.md"),
            header + self._extract_section(records, "architecture"),
        )
    ...
    def _generate_getting_started(self, records: list[dict[str, Any]]) -> None:
        ...
        self._write_doc(
            self._synth_filename("GETTING_STARTED.md"),
            header + self._extract_section(records, "entry_points"),
        )
    ...
    def _generate_concepts(self, records: list[dict[str, Any]]) -> None:
        ...
        header += (
            "## Key Abstractions\n\n"
            "This document covers fundamental concepts and design patterns "
            "used in the codebase.\n\n"
        )
        self._write_doc(
            self._synth_filename("CONCEPTS.md"),
            header + self._extract_section(records, "concepts"),
        )

    def _finalize_flat_output(self) -> None:
        """Inject navigation, generate GUIDE.md, and create backward-compat symlinks."""
        if not self.plan:
            return
        manifest = build_manifest(self.plan)
        inject_navigation_all_files(self.output_dir, manifest)
        guide_content = generate_guide_md(manifest, self.plan)
        guide_path = self.output_dir / "GUIDE.md"
    ...
    def _generate_top_down_agent(self) -> None:
        ...
        if self.plan:
            bottom_up_dir = self.base_output_dir / "output" / self.language
        else:
