...
# Preview changes without running analysis
lantern diff --repo /path/to/repo
...
| Scenario | Handling |
|----------|----------|
| Not a git repo | Fallback: mtime/hash comparison of all files |
| Stored SHA doesn't exist (force push/rebase) | Detect failure, fallback to full re-analysis |
...
1. **`core/diff_tracker.py`** — `DiffTracker` class (git operations + impact calculation)
2. **Extend `ExecutionState`** — Add `git_commit_sha` and `file_manifest` fields
3. **Extend `StateManager`** — Methods for cleaning/updating `.sense` files, saving
   git commit SHA
4. **`cli/main.py`** — Add `update` and `diff` commands
5. **Modify `Runner`** — Support incremental batch execution (merge new/old `.sense`)
6. **Tests** — Unit tests for `DiffTracker`, integration tests for update flow
