...
### 📄 規格書感知文檔
以設計規格書豐富生成的文檔內容：
- `lantern spec add spec.pdf` — LLM 自動配對規格書與程式碼模組
- 支援 PDF（含表格萃取）與 Markdown
- 規格書脈絡注入分析、規劃與合成的提示詞中
- 配對結果存於 `.lantern/specs.toml` — 可自由編輯
...
# 關聯規格書與程式碼，產生更豐富的文檔
lantern spec add path/to/auth-spec.pdf        # 透過 LLM 自動配對規格書與模組
lantern spec list                              # 顯示所有規格書配對
lantern spec remove auth-spec                  # 移除規格書配對
```
...
| 指令 | 說明 |
| :--- | :--- |
| `lantern spec add <file>` | 新增規格書（PDF/MD），透過 LLM 自動配對模組，產生摘要 |
| `lantern spec list` | 列出 `.lantern/specs.toml` 中所有規格書配對 |
| `lantern spec remove <name>` | 移除規格書項目（可選刪除檔案） |
...
- [x] **規格書感知文檔**：關聯規格書（PDF/MD）與程式碼模組，產生具有設計意圖的豐富文檔。
